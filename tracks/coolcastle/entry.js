var trackFile = "./tracks/coolcastle/coolcastleM.glb"
var trackName = "Cool Castle Canyon"
var trackShadow = "./tracks/coolcastle/ShadowN3.png"
var startPos = new THREE.Vector3(0,0.3,0)
var startTheta = 1.6
var numLaps = 3
var floorName = ["RoadStart","rCanyon","BCanyonBoost","rCarpet","rDirt","rSand","rWoodStart","BWoodTrick","DWater","canyon0_1_34_SKP__rockOrange_m","OSideRoadLeft","OSideRoadright","DSideWater","canyon0_1_34_SKP__sandStone","rGenerous","tRock","tJumps"]
var wallName = ["RoadWalls","canyonWalls","canyon0_1_34_SKP__stoneTile","SideRoad","rockyBlocky"]
var water, swater, cube
var boosts = ["BCanyonBoost","BWoodTrick"]
var materials = [];
var floor = []
var walls = []
...
//Helpers
var shadHelper = new THREE.CameraHelper(directionalLight.shadow.camera);
//scene.add( shadHelper );
...
scene.background = new THREE.CubeTextureLoader()
					.setPath( 'tracks/coolcastle/mp_marvelous/' )
...
loader.load(trackFile, function(gltf, err) {
  ...
  for (var i = 0; i < floorName.length; i++) {
    var item = root.getObjectByName(floorName[i]);
    if(item.name[0]=='O'){
      item.speed = 0.4
    }else{
      item.speed = 1
  ...
  for (var i = 0; i < wallName.length; i++) {
    walls.push(root.getObjectByName(wallName[i]))
  }
...
function finalize(){
  //afterimagePass = new THREE.AfterimagePass( 0.96 );
